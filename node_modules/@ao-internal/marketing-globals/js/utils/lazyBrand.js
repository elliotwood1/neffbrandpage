...
export default function lazyBrandInit() {
	...
	const hasLazyImages = document.querySelector('.lazy-brand-image');
	const hasBgLazyImages = document.querySelector('.lazy-brand-image-bg');

	if (typeof window.LazyScroll !== 'undefined') {
		const lazyImagesInstance = hasLazyImages ? new LazyScroll('.lazy-brand-image') : false;
		const lazyBgImagesInstance = hasBgLazyImages ? new LazyScroll('.lazy-brand-image-bg') : false;
		...
		if (lazyImagesInstance && lazyImagesInstance.images || lazyBgImagesInstance && lazyBgImagesInstance.images) {
